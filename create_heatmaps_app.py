...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
		...
		#model.eval()
        with torch.no_grad():
            features2 = features2.expand(1, -1, -1)
            ...
            with torch.no_grad():
                ...
                for layer in model.layers2:
                    ...
                    B = B + B_
                B = model.normA(B)
                features2 = model.DFF(A.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1),B.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1).permute(0, 2, 1)#给特征加权重.permute(0, 2, 1, 3)
                A = model.classifier(features2)  # [B, n_classes]
                #A = torch.topk(A, 1, dim=2)[0]
                A = A[:, :, 3].unsqueeze(2)
    ...
    with open(os.path.join(p_slide_save_dir, 'risk.csv'), 'w') as file:  
        file.write('Risk\n')  # 写入列名，通常CSV中需要列名  
        file.write(f'{risk}\n')  # 写入 risk 的值  